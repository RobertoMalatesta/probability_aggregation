...
%One such observation is related to the aggregation of multiple probabilities. 
...
%\vspace{-2em}
%\centering
%\hspace*{-3.5em} 	\includegraphics{Figures/LegendExamplePlot} % requires the graphicx package
%\vspace{-3.5em}
%
%\hspace{-0.5em}
...
\caption[Optional caption for list of figures]{Scatterplots of the probability forecasts given for two questions in our dataset. }
...
%If the analysis requires a time-dependent $\boldsymbol{b}$, a hidden process similar to (\ref{hiddenpr}) can be defined for $\boldsymbol{b}$. 

%As a final note, the bias term $\boldsymbol{b}$ can be made time-dependent by adding another regression equation $\boldsymbol{b}_t \sim \mathcal{N}(\delta \boldsymbol{b}_{t-1}, \sigma^2_b)$ to the model. 
...
%This means that $X_{t,k} = X_{t,k}(1)/\beta$ when $\beta$ is equal to the true value of $b_3$. 
...
\end{eqnarray*}
%Given that it is not clear which rule should be used for predicting geopolitical events, the SAC-procedure is evaluated  separately under both rules in Sections \ref{syntheticData} and \ref{realData}. Once $\hat{\beta}$ has been computed, 
...
\begin{figure}[h!]
\vspace{-5em}
\hspace{-25em}	\includegraphics{Figures/LegendSynthetic} % requires the graphicx package
 \includegraphics[width= 0.49\textwidth]{Figures/SyntheticBeta}
\caption[Optional caption for list of figures]{
%Comparing SAC that optimizes over the logarithmic score ($\text{SAC}_{\text{LOG}}$), SAC that optimizes over the Brier score ($\text{SAC}_{\text{BRI}}$), and the Exponentially Weighted Moving Average (EWMA) under synthetic data. 
The marginal effect of $\beta$ on the average quadratic loss.}
\label{Synthetic}
\end{figure}


%$|| \log(\boldsymbol{b}) - \log(\hat{\boldsymbol{b}})||^2/5$, respectively. The latter is measured in the log-space to allow a fair comparison of any discrepancies between under- and over-confident bias-vectors. 
...
%
%\vspace{0em}
%\hspace{2.5em}Hidden Process
%\vspace{-0.5em}
%
%\subfigure{
%\hspace{-1em} \includegraphics[width = 0.32\textwidth]{Figures/SyntheticSigma2}
%\label{SyntheticSigma2}
%}
%\subfigure{
%\hspace{-1.5em} \includegraphics[width = 0.32\textwidth]{Figures/SyntheticBeta}
%\label{SyntheticBeta}
%}
%\subfigure{
%\hspace{-1.5em} \includegraphics[width = 0.32\textwidth]{Figures/SyntheticK}
%\label{SyntheticK}
%}
%
%\vspace{0.5em}
%\hspace{2.5em}Bias Vector
%\vspace{-0.5em}
%
%\subfigure{
%\hspace{-1em}  \includegraphics[width = 0.32\textwidth]{Figures/SyntheticSigma2bias}
%\label{SyntheticSigma2b}
%}
%\subfigure{
%\hspace{-1.5em}  \includegraphics[width = 0.32\textwidth]{Figures/SyntheticBetabias}
%\label{SyntheticBetab}
%}
%\subfigure{
%\hspace{-1.5em}   \includegraphics[width = 0.32\textwidth]{Figures/SyntheticKbias}
%\label{SyntheticKb}
%}
%
%
%\caption[Optional caption for list of figures]{
%%Comparing SAC that optimizes over the logarithmic score ($\text{SAC}_{\text{LOG}}$), SAC that optimizes over the Brier score ($\text{SAC}_{\text{BRI}}$), and the Exponentially Weighted Moving Average (EWMA) under synthetic data. 
%The top and bottom rows present the accuracies to capture the hidden process and the bias vector, respectively. }
%\label{Synthetic}
%\end{figure}

%Figure \ref{Synthetic} summarizes the results in a series of plots. The first row presents the marginal effects of the three grid variables on
...
%The accuracy of the estimated hidden process and bias vector are measured with the average quadratic loss in the probability space
%\begin{eqnarray}
%\sum_{k=1}^K \sum_{t=1}^{100} ( \logit^{-1}(\hat{X}_{t,k}) - \logit^{-1}(X_{t,k}))^2 / (100K) \label{loss1}
%\end{eqnarray}
% and the average quadratic loss, $|| \boldsymbol{b} - \hat{\boldsymbol{b}}||^2/5$, respectively.

%Given that the plots were computed by choosing one grid variable (e.g., $\beta$) at a time and averaging over the remaining two variables  (e.g., $K$ and $\sigma^2$), each value in the plots represents an average of a $40 \times 5^2 = 1,000$ values. 
%
%As the Y-scale in Figure \ref{Synthetic} is very small, all three methods estimate the hidden process very accurately. Generally EWMA performs slightly better than $\text{SAC}$. This result is due to its 6-parameter calibration function (\ref{EWMA}) that is more flexible than the 1-parameter calibration made by $\text{SAC}$. 
%
%Apart from $\text{SAC}_{\text{LOG}}$ outperforming $\text{SAC}_{\text{BRI}}$ uniformly across all parameter values, the two approaches behave very similarly.  They suffer  from the increasing level of noise in the expert logit-probabilities, estimate the bias vector less accurately when $\beta$ is large, and, unlike EWMA, make greater use of the increasing number of questions.  EWMA is able to outperform SAC only when the bias among the experts is very small. 
...
%If $\boldsymbol{p}_{t,k}$ is a vector collecting all the probability forecasts made for the $k$th question at time $t$, then 
%
...
\end{eqnarray*}
\normalsize
%Therefore it serves a similar role to the bias vector found under \textit{Sample-And-Calibrate}. 
%The term $\boldsymbol{e}_{i,k}$ is a vector of length 5 indicating which level of self-reported expertise the $i$th expert in the $k$th question belongs to. For instance, if $\boldsymbol{e}_{i,k} = [0, 1, 0, 0, 0]$, then the expert identifies himself with the expertise level two. 
...
%As a result, the aggregate forecasts are either very close to the correct answer or very far from it.  
...
\subfigure[\scriptsize SDLM ]{
\hspace*{-2em}  \includegraphics[width = 0.286\textwidth]{Figures/CalibrationNONE10}
\label{CalibrationNONEa}
}
\subfigure[\scriptsize $\text{SAC}_{\text{LOG}}$ ]{
\hspace*{-1.15em} \includegraphics[width = 0.235\textwidth]{Figures/CalibrationLOG10}
\label{CalibrationPLATTLOG10}
}
\subfigure[\scriptsize $\text{SAC}_{\text{BRI}}$ ]{
\hspace*{-1.15em} \includegraphics[width = 0.235\textwidth]{Figures/CalibrationBRI10}
\label{CalibrationPLATTBRI10}
}
\subfigure[\scriptsize $\text{BSAC}_{\text{LOG}}$ ]{
\hspace*{-1.15em} \includegraphics[width = 0.286\textwidth]{Figures/CalibrationFULL10}
\hspace*{-2em} \label{CalibrationPLATTFULL10}
}


\subfigure[ \scriptsize SDLM]{
 \hspace*{-2em} \includegraphics[width = 0.286\textwidth]{Figures/CalibrationNONE}
\label{CalibrationNONEb}
}
\subfigure[\scriptsize $\text{SAC}_{\text{LOG}}$]{
\hspace*{-1.15em} \includegraphics[width = 0.235\textwidth]{Figures/CalibrationSTO-Log}
\label{CalibrationSTC-Log}
}
\subfigure[ \scriptsize $\text{SAC}_{\text{BRI}}$]{
\hspace*{-1.15em} \includegraphics[width = 0.235\textwidth]{Figures/CalibrationSTO-Bri}
\label{CalibrationSTC-Bri}
}
\subfigure[\scriptsize  $\text{BSAC}_{\text{LOG}}$]{
\hspace*{-1.15em} \includegraphics[width = 0.286\textwidth]{Figures/CalibrationSO}
\hspace*{-2em}\label{CalibrationBSTC-Log}
} 
%\restoregeometry
\caption[Optional caption for list of figures]{The top and bottom rows show in- and out-of-sample calibration and sharpness, respectively. 
%The models are Simple Dynamic Linear Model (SDLM), the \textit{Sample-And-Calibrate} approach that optimizes over the logarithmic score ($\text{SAC}_{\text{LOG}}$), the \textit{Sample-And-Calibrate} approach that optimizes over the Brier score ($\text{SAC}_{\text{BRI}}$), and the fully-Bayesian version of the \textit{Sample-And-Calibrate} approach that optimizes over the logarithmic score ($\text{BSAC}_{\text{LOG}}$).
}
...
%The solid line represents the posterior mean of the calibrated crowd belief as estimated by $\text{SAC}_{\text{LOG}}$. The dashed lines connect the point-wise 95\% posterior intervals across different time points. 
...
%The model estimation is performed in two steps: The first step, known as the \textit{sampling step}, samples constrained versions of the model parameters via Gibbs sampling. The sampling is done from standard distributions with fast convergence to the target distribution (see Online Supplement for technical details). The second step, known as the \textit{calibration step}, uses a one-dimensional optimization procedure to transform the constrained parameter values to their unconstrained counterparts.
...
%If the experts were asked to provide personal information, it may be of interest to study the forecasting behavior of different kinds of experts. For instance, 
%
...
%If the experts updated their probability forecasts very frequently, it is possible to use the model to estimate expert-specific parameters such as $\sigma_{k,i}^2$ or $b_{i,k}$. Together these estimates can be used to separate the accurate from the inaccurate forecasters. The estimates can also provide insight on the responsiveness and  
