...
We use this procedure to generate the hidden processes for $K$ questions with $T_k = 100$ for $k = 1, \dots, K$. Each question involves 50 experts allocated evenly among the five expertise groups. The expert logit-probabilities $Y_{t,k}$ are generated from Equation (\ref{observedpr}) by applying bias and noise to the hidden process $X_{t,k}$. Similarly to the real-world data setting discussed in Section \ref{realData}, extreme logit-probabilities are truncated to avoid overly influential observations; that is, any $p_{i,t,k} = \logit^{-1}(Y_{i,t,k})$ less than 0.01 (or greater than 0.99) is truncated to 0.01 (or to 0.99). 


The simulation is then performed over a three-dimensional grid of parameter values:
\begin{eqnarray*}
\beta &\in& \{1/2, 3/4, 1, 4/3, 2/1\}\\
\sigma^2 &\in& \{0.5, 1.0, 1.5, 2.0, 2.5\}\\
K &\in& \{20, 35, 50, 65, 80\},
\end{eqnarray*}
where $\beta$ generates the bias vector by  $\boldsymbol{b} = [0.50, 0.75, \dots, 1.50]^T \beta$. 
The simulation runs a total of 40 times at every grid point. Each run records the accuracy to which the approaches are able to estimate the hidden process and bias vector. The first is measured in terms of the average quadratic loss in the probability space, $\sum_{k=1}^K \sum_{t=1}^{T_k} (\hat{p}_{t,k} - \logit^{-1}(X_{t,k}))^2 / \sum_{k=1}^K T_k$, while the latter is measured in terms of the average quadratic loss, $|| \boldsymbol{b} - \hat{\boldsymbol{b}}||^2/5$.
