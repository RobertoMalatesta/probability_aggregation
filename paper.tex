...
\end{mydef}
\noindent
Even though it is unlikely that any single expert has all the available information, a large and diverse group of experts may share a considerable portion of this information. The so called \textit{wisdom of the crowd} is therefore an attractive proxy for the amount of information available at any given time point. 

If a model is to efficiently use this proxy for estimating the hidden process $X_{t,k}$, it must be able to discriminate between noise and signal as experts may believe in false information, hide their true beliefs on an ethical basis, or be biased for many other reasons. Since in our model the experts are assumed to be, on average, a multiplicative constant $\boldsymbol{b}$ away from the calibrated crowd belief, an individual element of $\boldsymbol{b}$ can be interpreted as a group-specific \textit{systematic bias} that labels the group either as over-confident $\left( b_j \in (1, \infty)\right)$ or as under-confident $\left(b_j \in (0,1)\right)$.  Due to the high sparsity of our data, estimating a bias term and analyzing the bias separately for each expert is not possible. See Section \ref{ExpertBias} for an analysis and discussion on the bias terms. Any other deviation from the calibrated crowd belief is considered \textit{random noise}. This noise is measured in terms of $\sigma^2_k$ and can be assumed to be caused by momentary over-optimism (or pessimism), false beliefs, or other misconceptions. 
...
\end{eqnarray*}
gives a sequence of $T_k$ calibrated logit-probabilities for the event $Z_k = 1$. Repeating this procedure for $K$ times, we generate the hidden process for $K$ questions, each with a time horizon of $T_k = 101$. The questions involve 25 experts allocated evenly among five expertise groups. Since the event resolves at time $t = 101$, the expert logit-probabilities $Y_{i,t,k}$ are only given for times $t = 1$ to $100$. These are generated from Equation (\ref{observedpr}) by applying bias and noise to the hidden process $X_{t,k}$. The simulation then iterates over a three-dimensional grid of parameter values
\begin{eqnarray*}
\sigma^2 &\in& \{1/2, 1, 3/2, 2, 5/2\}\\
\beta &\in& \{1/2, 3/4, 1, 4/3, 2/1\}\\
K &\in& \{20, 40, 60, 80, 100\},
\end{eqnarray*}
where $\beta$ gives the value of the bias vector by letting $\boldsymbol{b} = [1/2, 3/4, 1, 4/3, 2/1]^T \beta$. Each grid point is used  40 times to generate a synthetic dataset. This dataset is then used to assess how accurately \textit{Sample-And-Calibrate} estimates the hidden process and bias vector. The accuracy of the estimated hidden process is measured with the average quadratic loss in the probability space, $\sum_{k=1}^K \sum_{t=1}^{T_k} ( \logit^{-1}(\hat{X}_{t,k}) - \logit^{-1}(X_{t,k}))^2 / \sum_{k=1}^K T_k$, and the accuracy of the estimated bias vector is measured with the average quadratic loss, $|| \boldsymbol{b} - \hat{\boldsymbol{b}}||^2/5$.
...
\caption[Optional caption for list of figures]{Comparing \textit{Sample-And-Calibrate} that optimizes over the logarithmic score ($\text{SAC}_{\text{LOG}}$) and \textit{Sample-And-Calibrate} that optimizes over the Brier score (SAC-Bri) under synthetic data. The top row presents the accuracy to capture the hidden process. The bottom row presents the accuracy to capture the bias vector. }
\label{Synthetic}
\end{figure}

Figure \ref{Synthetic} summarizes the results in a series of plots. The first row shows the marginal effects of the three grid variables on the accuracy to which the \textit{Sample-And-Calibrate} under the logarithmic loss ($\text{SAC}_{\text{LOG}}$) and the Brier score (SAC-Bri) are able to estimate the hidden process.  The second row is similar in nature but measures the accuracy of estimating the bias vector instead.  Since the plots were computed by choosing one grid variable (e.g. $\beta$) at a time and averaging over the remaining two variables  (e.g. $K$, and $\sigma^2$), each value in the plots represents an average of a $40 \times 5^2 = 1,000$ values. 

Based on the first row of plots, $\text{SAC}_{\text{LOG}}$ and SAC-Bri estimate the hidden process to a similar accuracy. Even though some relative differences are visible, the loss values are very small in absolute terms, hence suggesting good overall accuracy to capture the hidden process. Estimating the hidden process, however, becomes more difficult as either $\sigma^2$ or $\beta$ gets larger. The latter seems more unintuitive and can be understood better by observing that larger values of $\beta$ lead to more extreme expert logit-probabilities and hence to more frequent truncation. This causes the estimation accuracy to degrade because  a high level of truncation results to a drastic deviation from the model assumptions. Increasing the number of questions improves the estimation accuracy but only very little.


Based on the second row of plots, $\text{SAC}_{\text{LOG}}$ and SAC-Bri  capture the bias vector rather precisely as long as the true values of $\beta$ and $\sigma^2$ are not very large. This behavior can be again considered as being caused by the truncation of the extreme expert logit-probabilities. Surprisingly, even though $\text{SAC}_{\text{LOG}}$ is more accurate in estimating the bias vector, it is unable to outperform SAC-Bri in estimating the hidden process. 
