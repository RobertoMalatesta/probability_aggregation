...
\end{eqnarray*}
where the trailing input parameter emphasizes the constraint $b_{3} = 1$. Since this version is identifiable, estimates of the model parameters can be obtained. Denote the estimates by placing a hat on the parameter symbol. For instance, $\hat{\boldsymbol{b}}(1)$ and $\hat{X}_{t, k}(1)$ represent the estimates of $\boldsymbol{b}(1)$ and $X_{t, k}(1)$, respectively. These estimates are found via Gibbs sampling that only makes use of standard distributions. See Appendix \ref{appendix} for the technical details of our sampler, and, e.g., \citet{gelman2003bayesian} for a discussion on the general principles of Gibbs sampling. 
...
K &\in& \{20, 35, 50, 65, 80\},
\end{eqnarray*}
where $\beta$ generates the bias vector  $\boldsymbol{b} = [0.50, 0.75, \dots, 1.50]^T \beta$. These values were considered to be realistic based on our experience with the real-world probability forecasting data. Each triple of parameter values,  i.e. a grid point, is used  40 times to generate a synthetic dataset. This dataset is then used to assess how accurately the approaches are able to estimate the hidden process and bias vector. The accuracy to estimate the hidden process is measured with the average quadratic loss in the probability space, $\sum_{k=1}^K \sum_{t=1}^{T_k} ( \logit^{-1}(\hat{X}_{t,k}) - \logit^{-1}(X_{t,k}))^2 / \sum_{k=1}^K T_k$, and the accuracy to capture the bias vector is quantified by the average quadratic loss, $|| \boldsymbol{b} - \hat{\boldsymbol{b}}||^2/5$.
