...
probability judgement from each expert, even though it is common for
experts studying real problems to update their probability estimates
over time.  This paper makes an advance towards unexplored areas of
probability aggregation by considering a dynamic context in which
experts are allowed to update their beliefs at random intervals.  We
demonstrate our method on a real-world dataset that includes over
2,300 experts making multiple probability forecasts on different
subsets of 166 different international political events. We present a
dynamic model that takes into account the forecaster’s level of
self-reported expertise and produces aggregate probabilities that are
sharp and well-calibrated both in- and out-of-sample. Using the
aggregate probabilities as the no-bias reference-line, the lower the
self-reported expertise in a group, the more under-confident it is
found to be.
\end{abstract}
...
\end{eqnarray}
where $S$ is a strictly proper scoring rule such as the negative Brier score (\citet{Brier})
\begin{eqnarray*}
