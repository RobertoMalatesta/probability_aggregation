...
\section{Discussion}
This paper began with an introduction of a rather unorthodox but nonetheless realistic time-series setting in which probability forecasts are made very infrequently by a heterogeneous group of experts. The resulting data is too sparse to be modeled efficiently with standard time-series methods.  In response to the lack of appropriate modeling procedures, our work introduces an interpretable time-series model that incorporates self-reported expertise to capture a sharp and well-calibrated estimate of the crowd belief. The model is estimated in two-steps: The first step, known as the \textit{sampling step}, samples constrained versions of the model parameters via a Gibbs sampler that only makes use of standard distributions (see Appendix \ref{appendix} for technical details). The second step, known as the \textit{calibration step}, involves a one-dimensional optimization procedure that transforms the constrained parameter values to their unconstrained counterparts. 
